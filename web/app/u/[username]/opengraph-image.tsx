...
// Per-tier accent for the unfurl (gold reserved; orange is the brand).
...
async function tryCard(username: string): Promise<Card | null> {
  ...
  return "card" in res ? res.card : null;
}
...
export default async function Image({ params }: { params: Promise<{ username: string }> }) {
  ...
  const accent = TIER_ACCENT[card.finish] ?? "#ffa116";
        ...
        <div style={{ display: "flex", flexDirection: "column", flex: 1 }}>
          ...
          <div style={{ display: "flex", marginTop: 20, fontSize: 34, fontWeight: 700 }}>
            <span style={{ display: "flex", color: accent }}>
              {card.overall} {FINISH_LABEL[card.finish]}
            </span>
