...
export async function loadCardAssets(card: Card, w: number) {
  ...
  const [fonts, bg, avatar, flag] = await Promise.all([
    loadCardFonts(),
  ...
  return { fonts, bg, avatar, flag, avW, avH };
}
...
export function cardTree(card: Card, assets: CardAssets, w: number) {
  ...
  const { bg, avatar, flag, avW, avH } = assets;
...
// The standalone embeddable card image: leetfut.com/<user>.png.
