...
// Tiers that earn the full spectacle (Chrome + Icon/Legend). TOTW (in-form) also
// gets a modest burst because it is, by definition, a rare "event" card. Red
// (80-84) does NOT burst — same treatment as Gold/Silver/Bronze, since it's a
// plain threshold tier, not a rare/gated one.
const BURST_TIERS: ReadonlySet<Finish> = new Set<Finish>(["chrome", "icon", "totw"]);
