...
// juggles the WC26 ball; a football-LeetCode pun rotates every ~1.8s.
  ...
  return (
    <main className="relative z-[2] flex h-[100dvh] flex-col items-center justify-center px-6 text-center">
      ...
      <div className="font-display mt-8 text-[clamp(30px,5vw,52px)] leading-none tracking-[.02em] text-ink">
        SCOUTING{" "}
        {username && <span className="font-mono align-middle text-[0.5em] text-brand">@{username}</span>}
      </div>
      ...
      <div className="mt-7 h-[3px] w-[min(260px,70vw)] overflow-hidden rounded-full bg-white/10">
        <div
          ...
          style={{ animation: "lf-load 1.3s ease-in-out infinite" }}
        />
      </div>

      <style>{`@keyframes lf-load{0%{transform:translateX(-120%)}100%{transform:translateX(360%)}}`}</style>
    </main>
  );
}
