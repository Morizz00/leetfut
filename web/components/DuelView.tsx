...
function StatBar({
  ...
  return (
    <div
      ...
      style={
        resolved
          ? { animation: "lf-row-resolve .45s cubic-bezier(.16,1,.3,1) both" }
          : { opacity: 0.3 }
      }
    >
...
function ScoreDigit({ value, accent }: { value: number; accent: string }) {
  return (
    <span
      ...
      style={{
        color: accent,
        animation: "lf-score-tick .38s cubic-bezier(.16,1,.3,1) both",
      }}
    >
      {value}
    </span>
...
export default function DuelView({ duel }: { duel: Duel }) {
  ...
  const { canNativeShare, nativeShare, copyLink, linkCopied } = useShareActions({
    getSharePayload: () =>
      duelSharePayload(challenger.username, opponent.username),
    getIntentUrl: () => duelIntentUrl(challenger.username, opponent.username),
    getCopyUrl: () => duelUrl(challenger.username, opponent.username),
  });
  ...
  const nameLen = Math.max(challenger.username.length, opponent.username.length);
      ...
      >
        ...
        <div
          ...
          style={{
            ...
            animation: settled
              ? undefined
              : "lf-radar-in .7s cubic-bezier(.16,1,.3,1) .5s both",
          }}
        >
          <div className="w-[78%]">
            <StatRadar
                ...
                >
                    ...
                    {winnerCard ? (
                      <div
                        ...
                        style={{
                          ...
                          animation:
                            "lf-name-shimmer 3.4s ease-in-out .25s both",
                        }}
                      >
                  ...
                  <span
                    ...
                    style={{
                      ...
                      animation:
                        "lf-underline .55s .18s cubic-bezier(.16,1,.3,1) both",
                    }}
                  />
              ...
              <div className="mt-[10px] flex flex-col gap-[8px]">
                {canNativeShare && (
                  <button
                    type="button"
                    onClick={nativeShare}
                    className="font-display group relative flex h-[46px] w-full items-center justify-center gap-[9px] overflow-hidden rounded-xl bg-gradient-to-b from-brand to-brand-mid text-[17px] tracking-[.05em] text-[#1a0f00] shadow-[0_0_0_1px_rgba(255,161,22,.45),0_10px_28px_-6px_rgba(255,161,22,.5)] transition-all duration-200 ease-out hover:-translate-y-[1px] active:translate-y-0 active:scale-[.985]"
                  >
                    <span
                      aria-hidden
                ...
                <div className="grid w-full grid-cols-3 gap-[8px]">
                  <button
                    ...
                    onClick={() =>
                      window.open(
                        duelIntentUrl(challenger.username, opponent.username),
                        "_blank",
                        "noopener,noreferrer",
                      )
                    }
                  ...
                  <Link
                    href={`/${opponent.username}/vs/${challenger.username}`}
