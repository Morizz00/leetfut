...
// card (x/540, y/820); font sizes are cqw (px/540×100) so the card scales with
// its container. The tier background PNG carries the artwork; everything else
// (rating, name, stats, flag, separator lines) is overlaid here.
...
// value/label positions per stat (col x115/305 + label gap 60; rows 530/592/654, labels +5).
const STAT_CELLS: {
  k: StatKey;
  l: string;
  vx: number;
...
// Separator lines drawn over the blank card art. [left%, top%, width%].
...
function PlayerCard({ card }: { card: Card }) {
  ...
  return (
    <div className="leetfut-card-frame" style={wrap}>
      {/* tier background art */}
      <img
        src={t.bg}
        alt=""
      ...
      {/* Avatar — clipped to the card silhouette (the bg PNG used as a mask) so
          ...
          any photo blends in like a FUT cut-out. */}
      <div
        style={{
          position: "absolute",
          inset: 0,
      ...
      {/* country flag — only when a country was resolved from LeetCode */}
      {card.country && (
        <img
      ...
      {/* signature — maker's mark (bottom-left) + handle (bottom-right) on the
          ...
          clone (CardActions tags it `.leetfut-capturing`), so it lands in every
          download / copy / share image but never on screen. See lib/capture.ts
          + the `.leetfut-signature` rule in globals. */}
      <div className="leetfut-signature">
        <div
          style={{
            ...at(8, 94.8),
            fontFamily: FONT_BOLD,
