...
describe("reveal sequencer", () => {
  it("common tiers (bronze/silver/gold/red) rise → ignite → freeze, no burst", () => {
    for (const f of ["bronze", "silver", "gold", "red"] as Finish[]) {
      expect(phases(f)).toEqual(["rise", "ignite", "freeze"]);
      expect(hasBurst(f)).toBe(false);
    }
  });

  it("rare event tiers (totw/chrome/icon) include the burst", () => {
    for (const f of ["totw", "chrome", "icon"] as Finish[]) {
  ...
  it("step timings are strictly increasing", () => {
    const steps = sequenceFor("chrome", false);
  ...
  it("reduced motion collapses every tier to a single freeze at 0ms", () => {
    for (const f of ["bronze", "gold", "icon", "chrome"] as Finish[]) {
